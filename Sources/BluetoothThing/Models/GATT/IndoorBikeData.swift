...
public struct IndoorBikeData: GATTCharacteristic {
    ...
    public let averageSpeed = GATTData(bytes: 2, decimalExponent: -2, resolution: 0.01, unit: UnitSpeed.kilometersPerHour)
    public let instantaneousCadence = GATTData(bytes: 2, decimalExponent: -1, resolution: 0.5, unit: UnitFrequency.rpm)
    public let averageCadence = GATTData(bytes: 2, decimalExponent: -1, resolution: 0.5, unit: UnitFrequency.rpm)
    public let totalDistance = GATTData(bytes: 3, unit: UnitLength.meters)
    public let resistanceLevel = GATTData(bytes: 2)
    public let instantaneousPower = GATTData(bytes: 2, unit: UnitPower.watts)
    public let averagePower = GATTData(bytes: 2, unit: UnitPower.watts)
    public let totalEnergy = GATTData(bytes: 2, unit: UnitEnergy.kilocalories)
    public let energyPerHour = GATTData(bytes: 2, unit: UnitEnergy.kilocalories)
    public let energyPerMinute = GATTData(bytes: 1, unit: UnitEnergy.kilocalories)
    public let heartRate = GATTData(bytes: 1, unit: UnitHeartBeat.bpm)
    public let metabolicEquivalent = GATTData(bytes: 2, decimalExponent: -1, resolution: 0.1)
    public let elapsedTime = GATTData(bytes: 2)
    public let remainingTime = GATTData(bytes: 2)
